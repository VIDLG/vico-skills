...
class WilcoAutomationTests(unittest.TestCase):
    ...
    def test_repo_docs_define_persistence_policy_and_common_paths(self) -> None:
        ...
        self.assertIn("Let `wilco-feedback` classify it and draft the issue.", readme)
        ...
        self.assertIn("direct_execute -> wilco-plan", readme)
        self.assertIn("wilco-plan -> direct_execute", readme)
        self.assertIn("wilco-probe -> direct_execute", readme)
        ...
        self.assertIn("让 `wilco-feedback` 自动归类并生成 issue 草稿。", readme_zh)
        ...
        self.assertIn("direct_execute -> wilco-plan", readme_zh)
        self.assertIn("wilco-plan -> direct_execute", readme_zh)
        self.assertIn("wilco-probe -> direct_execute", readme_zh)
        ...
        self.assertIn("`wilco-plan -> wilco-exec`", contracts)
        ...
        self.assertIn("`wilco-plan -> wilco-exec`", contracts_zh)
    ...
    def test_plan_contract_defines_execution_readiness_and_probe_consumption_hints(self) -> None:
        ...
        self.assertIn("`wilco-plan -> wilco-exec`", plan_verify)
    ...
    def test_probe_textual_outputs_follow_user_language_while_preserving_stable_literals(self) -> None:
        ...
        self.assertIn("how do I use wilco-probe", probe_skill)
        ...
        self.assertIn("surface `Skill route` and `Route reason` in the first visible update when `wilco-probe` is selected", probe_help)
    ...
    def test_probe_explicit_submodes_bootstrap_scan_when_probe_state_is_missing(self) -> None:
        ...
        self.assertIn("`wilco-plan -> wilco-exec`", probe_skill)
    ...
    def test_exec_help_template_has_language_rule_and_axis_position(self) -> None:
        ...
        self.assertIn("show `Skill route` and `Route reason` in the first visible update when `wilco-exec` is selected", exec_help)
        self.assertIn("Axis position: the heavy end of the execution axis", exec_help)
    ...
    def test_exec_output_contract_covers_language_and_persistence(self) -> None:
        ...
        self.assertIn("how do I use wilco-exec", exec_skill)
