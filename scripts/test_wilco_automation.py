...
class WilcoAutomationTests(unittest.TestCase):
    ...
    def test_repo_docs_define_persistence_policy_and_common_paths(self) -> None:
        ...
        self.assertIn("Skill route: wilco-probe", readme)
        self.assertIn("Route reason: natural trigger", readme)
        self.assertIn("This repo does not ship a dedicated `npx` installer.", readme)
        self.assertIn("### Development Link", readme)
        self.assertIn("### Uninstall", readme)
        self.assertIn("### Optional Codex Installer Route", readme)
        ...
        self.assertIn("Route reason: natural trigger", readme_zh)
        self.assertIn("这个仓库本身不提供专门的 `npx` 安装器。", readme_zh)
        self.assertIn("### 开发期 Link", readme_zh)
        self.assertIn("### 卸载", readme_zh)
        self.assertIn("### 可选的 Codex Installer 路径", readme_zh)
