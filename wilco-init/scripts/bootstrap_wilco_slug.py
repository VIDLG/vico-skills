...
DOCS_SCRIPTS = SCRIPT_DIR.parents[1] / "wilco-docs" / "scripts"
import sys
...
def main() -> int:
    ...
    parser.add_argument("slug", help="Stable Wilco slug.")
    parser.add_argument("title", help="Human-readable task title.")
    parser.add_argument(
        "--level",
        choices=["no-doc", "plan-only", "prd-plan", "prd-plan-arch"],
        default="plan-only",
