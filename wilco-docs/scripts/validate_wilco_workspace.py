...
from wilco_common import detect_metadata, index_path, plan_path, prd_path, relative_path, repo_root_from_arg, resume_path


DATE_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$")
ACTIVE_PLAN_STATUSES = {"in_progress", "accepted", "draft"}
ARCHIVED_PLAN_STATUSES = {"archived", "cancelled", "superseded"}
ACTIVE_PRD_STATUSES = {"accepted", "in_progress", "draft"}
ARCHIVED_PRD_STATUSES = {"archived", "cancelled", "superseded"}
...
def validate_prd(path: Path, root: Path, errors: list[str]) -> None:
    ...
    if related_plan:
        require((root / related_plan).exists(), f"{path}: Related plan path does not exist: {related_plan}", errors)


def validate_archive_doc(path: Path, expected_statuses: set[str], errors: list[str]) -> None:
    meta = detect_metadata(path)
    require(meta.get("slug") == path.stem, f"{path}: slug must match filename", errors)
    require(meta.get("status") in expected_statuses, f"{path}: invalid archived status {meta.get('status')!r}", errors)
    require(is_iso_date(meta.get("updated")), f"{path}: Updated must be YYYY-MM-DD", errors)
    require(is_iso_date(meta.get("completed")), f"{path}: Completed must be YYYY-MM-DD", errors)
...
def validate_resume(path: Path, root: Path, errors: list[str]) -> None:
    ...
    if manifest and manifest != "optional .wilco/index/<slug>.json":
...
def validate_index(path: Path, root: Path, errors: list[str]) -> None:
    ...
    require(data.get("kind") == "wilco-linkage-v1", f"{path}: kind must be wilco-linkage-v1", errors)
    require(data.get("slug") == path.stem, f"{path}: slug must match filename", errors)
    state = data.get("state", {})
    require(state.get("status") in ACTIVE_PLAN_STATUSES | ARCHIVED_PLAN_STATUSES, f"{path}: invalid state.status {state.get('status')!r}", errors)
    ...
    allowed_keys = {"prd", "plan", "resume_current", "architecture", "plan_archive", "prd_archive"}
    for key in artifacts:
        require(key in allowed_keys, f"{path}: unexpected artifact key {key!r}", errors)
    for key in ("prd", "plan", "resume_current", "plan_archive", "prd_archive"):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the current .wilco workspace against Wilco document conventions.")
    ...
    for path in sorted((root / ".wilco" / "plans" / "active").glob("*.md")):
        if path.name != "README.md":
            validate_plan(path, root, errors)
    for path in sorted((root / ".wilco" / "prd" / "active").glob("*.md")):
        if path.name != "README.md":
            validate_prd(path, root, errors)
    for path in sorted((root / ".wilco" / "plans" / "archive").glob("*.md")):
        if path.name != "README.md":
            validate_archive_doc(path, ARCHIVED_PLAN_STATUSES, errors)
    for path in sorted((root / ".wilco" / "prd" / "archive").glob("*.md")):
        if path.name != "README.md":
            validate_archive_doc(path, ARCHIVED_PRD_STATUSES, errors)
    for path in sorted((root / ".wilco" / "resume").glob("*.md")):
        if path.name != "README.md":
            validate_resume(path, root, errors)
    for path in sorted((root / ".wilco" / "index").glob("*.json")):
        validate_index(path, root, errors)

    if errors:
        print("Wilco workspace validation failed:\n")
        for error in errors:
            print(error)
        return 1

    print("Wilco workspace validation passed.")
    return 0
