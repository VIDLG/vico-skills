...
- or the current temporary reconcile file under `.wilco/resume/<slug>.md`
...
- If more than one active slug exists and the intended execution target is not explicit, do not guess. Ask for an explicit slug or route back through `wilco-plan review`.
...
`wilco-exec` should support these explicit modes:
...
- Prefer `scripts/sync_wilco_index.py` for derived index refresh instead of hand-editing manifests.
...
- Use [references/help-template.md](references/help-template.md) for `wilco-exec help`.
