...
7. If no active slug is hit and the work should become tracked, hand off to `wilco-init` for slug bootstrap before writing the PRD.
8. If the work deserves a PRD, interview the user until the scope, interfaces, and tradeoffs are clear enough to write a durable PRD.
9. Identify the main modules, boundaries, and testing expectations without locking in fragile file-level detail.
10. Choose the stable topic slug and target file path: `.wilco/prd/active/<slug>.md`.
11. Update `.wilco/index/<slug>.json` only as a derived coordination layer for linked artifacts. Prefer `../wilco-docs/scripts/sync_wilco_index.py` when available instead of hand-editing derived linkage.
12. Write or update the PRD using the repository template in [references/prd-template.md](references/prd-template.md).
13. If the repo's docs structure is unclear or needs normalization, also use `wilco-docs`.
