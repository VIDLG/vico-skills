...
Use one stable split between Vico planning docs and long-lived project truth:
...
- `.vico/plans/active/`: default active documents. In `plan_only`, each plan carries both problem framing and execution. In `prd_backed`, plans focus on execution and link to the PRD.
- `.vico/prd/active/`: active PRDs for `prd_backed` slugs only.
- `.vico/resume/`: current-only handoff or reconciliation snapshots created on demand.
- `.vico/index/`: minimal machine-readable linkage manifests that can be rebuilt.
...
```text
.vico/prd/active/2026-04-09-action-pipeline.md
.vico/plans/active/2026-04-09-action-pipeline.md
.vico/resume/2026-04-09-action-pipeline.md
.vico/index/2026-04-09-action-pipeline.json
