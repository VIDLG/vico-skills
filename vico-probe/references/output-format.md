...
```md
vico-skills | workflow > lifecycle-transitions
...
```md
vico-skills | workflow > slug-lifecycle
Mode: concise | available: concise, detailed
Priority: critical
...
vico-skills | distribution > runtime-closure
Mode: concise | available: concise, detailed
Priority: critical
...
```md
vico-skills | overlap-and-rebuild > historical-scope
Mode: concise | available: concise, detailed
Priority: important
...
Write-back target
- .vico/README.md
- vico-plan/references/rules/routing.md
```

## Scan Example

```md
vico-skills | vico-probe > controller-gaps
Mode: concise | available: concise, detailed
Priority: critical

Intent overlay

- inspect current `vico-probe` design for contract gaps
...
- [important] Handoff stays aligned: handoff routing hints must stay aligned with `vico-plan`
- [detail] Stable split: `Findings` are the user-facing diagnostic summary; they do not need to mirror the full internal issue list item-for-item

Evidence

- the contract already distinguishes `default`, `scan`, `grill`, `review`, and `resolve`
- `vico-plan` already consumes `Target`, optional `Slug`, optional `Issue classes`, and the decision sections
...
- keep `Probe Handoff` aligned with `vico-plan`
...
```md
vico-skills | vico-probe > handoff-contract
Mode: concise | available: concise, detailed
Priority: important
...
- `Probe Handoff` should carry routing hints for `vico-plan`.
...
- prefer `resolve` to emit handoff when `vico-plan` is the next consumer
...
```md
vico-skills | vico-probe > routing-gaps
Mode: concise | available: concise, detailed
Priority: critical

Question 17: Should default `vico-probe` ask immediately when `ask-user` issues remain, while reserving `grill` for sustained interrogation?
...
```md
Current working topic map for vico-skills:
...
```md
vico-skills | vico-probe > stop-conditions
...
```md
vico-skills | vico-probe > recommendation-shortcut
Mode: detailed | available: concise, detailed
Priority: important
...
Suggested edits
- Update `vico-probe/SKILL.md` with the new output contract.
- Add `vico-probe/references/output-format.md` for long examples.
...
Slug
- 2026-04-11-vico-probe-contract
...
Suggested edits
- Rewrite `vico-plan/SKILL.md` as the front-door orchestrator.
