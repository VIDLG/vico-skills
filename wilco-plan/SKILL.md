...
Convert repository PRDs into active implementation plans that live under `docs/plans/active/`. For smaller and clearer tasks, support plan-only work without forcing a separate PRD. Keep the slug stable, capture status and dates in metadata, and prefer vertical slices over horizontal layer-by-layer plans.

## Workflow

1. Determine whether this plan is PRD-backed or plan-only.
2. If PRD-backed, confirm the source PRD path, ideally `docs/prd/active/<slug>.md`.
...
8. Write the plan to `docs/plans/active/<slug>.md` using [references/plan-template.md](references/plan-template.md).
